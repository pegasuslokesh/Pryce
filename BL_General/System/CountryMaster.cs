...
public class CountryMaster
{
    ...
    public CountryMaster(string strConString)
    {
        //
    ...
    public DataTable GetCountryMasterById(string CountryId,ref SqlTransaction trns)
    {
        ...
        paramList[1] = new PassDataToSql("@Country_Name", "", PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
        paramList[2] = new PassDataToSql("@optype", "2", PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
        dtInfo = daClass.Reuturn_Datatable_Search("sp_Sys_CountryMaster_SelectRow", paramList,ref trns);
    ...
    public DataTable getCountryCallingCode()
    {
        DataTable dtInfo = new DataTable();
        PassDataToSql[] paramList = new PassDataToSql[3];
        paramList[0] = new PassDataToSql("@Country_Id", "0", PassDataToSql.ParaTypeList.Int, PassDataToSql.ParaDirectonList.Input);
    ...
    {
        DataTable dt= daClass.return_DataTable("Select Country_Id from Sys_CountryMaster where Country_Name = '" + Name + "'");
        if(dt.Rows.Count>0)
        {
    ...
    public string getCountryIdByName(string Name,ref SqlTransaction trans)
    {
        DataTable dt = daClass.return_DataTable("Select Country_Id from Sys_CountryMaster where Country_Name = '" + Name + "'",ref trans);
        if (dt.Rows.Count > 0)
        {
            return dt.Rows[0][0].ToString();
        }
    ...
    public DataTable getCountryListForDDL()
    {
        DataTable dt = daClass.return_DataTable("select country_id,country_name from Sys_CountryMaster where isactive='true'");
        return dt;
    }
}
