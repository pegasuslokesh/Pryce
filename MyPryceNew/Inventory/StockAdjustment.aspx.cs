...
public partial class Inventory_StockAdjustment : BasePage
{
    ...
    StockAdjustmentReport stockReport = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        stockReport = new StockAdjustmentReport(Session["DBConnection"].ToString());
        ...
        if (!IsPostBack)
        {
            Common.clsPagePermission clsPagePermission = cmn.getPagePermission("../Inventory/StockAdjustment.aspx", HttpContext.Current.Session["CompId"].ToString(), HttpContext.Current.Session["UserId"].ToString(), HttpContext.Current.Session["Application_Id"].ToString());
            ...
            lblUnitCost.Text = SystemParameter.GetCurrencySmbol(LM.GetLocationMasterById(Session["CompId"].ToString(), Session["LocId"].ToString()).Rows[0]["Field1"].ToString(), Resources.Attendance.Unit_Cost, Session["DBConnection"].ToString());
    ...
    protected void btnEdit_Command(object sender, CommandEventArgs e)
    {
        ...
        if (dtStockEdit.Rows.Count > 0)
        {
            ...
            //Get sales Invoice no in case of replacement
            try
            {
                hdnSinvId.Value = "0";
                if (!string.IsNullOrEmpty(dtStockEdit.Rows[0]["field1"].ToString()) && dtStockEdit.Rows[0]["field1"].ToString() != "0")
                {
                    PassDataToSql[] paramList = new PassDataToSql[1];
                    paramList[0] = new PassDataToSql("@sinvId", dtStockEdit.Rows[0]["field1"].ToString(), PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
                    string sql = "select cm.Name,sih.invoice_no from Inv_SalesInvoiceDetail sid inner join Inv_SalesInvoiceHeader sih on sih.Trans_Id=sid.Invoice_No inner join Inv_ProductMaster pm on sid.Product_Id=pm.ProductId inner join Ems_ContactMaster cm on sih.Supplier_Id=cm.Trans_Id where sid.IsActive='true' and sih.trans_id=@sinvId";
                    using (DataTable _dt = objDa.GetDtFromQryWithParam(sql, paramList))
                    {
                        if (_dt.Rows.Count > 0)
                        {
                            hdnSinvId.Value = dtStockEdit.Rows[0]["field1"].ToString();
                            txtSinvRefOrderId.Text = dtStockEdit.Rows[0]["field2"].ToString();
                            txtSinvRefInvoiceNo.Text = dtStockEdit.Rows[0]["field3"].ToString();

                            txtCustomerName.Text = _dt.Rows[0]["Name"].ToString();
                            txtSinvNo.Text = _dt.Rows[0]["invoice_no"].ToString();
                            I2.Attributes.Add("Class", "fa fa-plus");
                            divProduct.Attributes.Add("Class", "box box-info collapsed-box");
                            I3.Attributes.Add("Class", "fa fa-minus");
                            divReplacement.Attributes.Add("Class", "box box-primary");
                        }
                    }
                }
            }
            ...
            if (dtTemp.Rows.Count > 0)
            {
                ...
                if (dtTemp.Rows.Count > 0)
                {
                    dtTemp = dtTemp.DefaultView.ToTable(false, "ProductId", "SerialNo", "Barcode", "BatchNo", "LotNo", "ExpiryDate", "Field1", "TransType", "TransTypeId", "ManufacturerDate", "Quantity", "Trans_Id", "Width", "Length", "Pallet_ID", "InOut");
    ...
    protected void btnSInquirySave_Click(object sender, EventArgs e)
    {
        ...
        Inv_SalesInvoiceHeader objSinvH = new Inv_SalesInvoiceHeader(HttpContext.Current.Session["DBConnection"].ToString());
        DataTable dtSinvSerial = new DataTable();
        if (hdnSinvId.Value != "0" && !string.IsNullOrEmpty(txtSinvNo.Text))
        {
            dtSinvSerial = ObjStockBatchMaster.GetStockBatchMaster_By_TRansType_and_TransId(Session["CompId"].ToString(), Session["BrandId"].ToString(), Session["LocId"].ToString(), "SI", hdnSinvId.Value);
        }
        ...
        if (ChkPost.Checked)
        {
            ...
            {
                double Totalqty = 0;
                string strInOut = ((HiddenField)gvr.FindControl("hdnInOutValue")).Value;
                ...
                if (Totalqty > 0)
                {
                    if (objProductM.GetProductMasterById(Session["CompId"].ToString(), Session["BrandId"].ToString(), ((Label)gvr.FindControl("lblgvProductId")).Text, HttpContext.Current.Session["FinanceYearId"].ToString()).Rows[0]["MaintainStock"].ToString() == "SNO")
                    {
                        ...
                        dt = new DataView(dt, "ProductId='" + ((Label)gvr.FindControl("lblgvProductId")).Text + "' and InOut='" + strInOut + "'", "", DataViewRowState.CurrentRows).ToTable();
                        if (dt.Rows.Count == 0)
                        {
                            DisplayMessage("Serial Information is not available for Product code = " + ((Label)gvr.FindControl("lblGvProductCode")).Text);
                            return;
                        ...
                        //check serial no for invoice replacement
                        if (((HiddenField)gvr.FindControl("hdnInOutValue")).Value == "I")
                        {
                            if (hdnSinvId.Value != "0" && !string.IsNullOrEmpty(txtSinvNo.Text) && dtSinvSerial.Rows.Count > 0)
                            {
                                using (DataTable dtInvoicedSerial = new DataView(dtSinvSerial, "productId=" + ((Label)gvr.FindControl("lblgvProductId")).Text, "", DataViewRowState.CurrentRows).ToTable())
                                {
                                    if (dtInvoicedSerial.Rows.Count == 0 && dt.Rows.Count > 0)
                                    {
                                        DisplayMessage("There was no serial associated with that sales invoice");
                                        return;
                                    }
                                    else if (dtInvoicedSerial.Rows.Count > 0 && dt.Rows.Count == 0)
                                    {
                                        DisplayMessage("Serial info mismatched with sales invoice");
                                        return;
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            foreach (DataRow drOld in dtInvoicedSerial.Rows)
                                            {
                                                if (drOld["SerialNo"].ToString().ToLower() == drNew["SerialNo"].ToString().ToLower())
                                                {
                                                    isValid = true;
                                                    continue;
                                                }
                                        ...
                                        if (isValid == false)
                                        {
                                            DisplayMessage("Serial mismatched with the sales invoice no");
                                            return;
                                        }
                                    }
                                }
        ...
        try
        {
            ...
            if (hdnStockTransId.Value != "0")
            {
                objAdjustHeader.UpdateAdjustHeader(Session["CompId"].ToString(), Session["BrandId"].ToString(), strLocationId, strLocationId, hdnStockTransId.Value, txtVoucherNo.Text, ObjSysParam.getDateForInput(txtVoucherDate.Text).ToString(), txtRemark.Text, txtNetAmount.Text, strPost, hdnSinvId.Value, txtSinvRefOrderId.Text, txtSinvRefInvoiceNo.Text, "", "", "True", DateTime.Now.ToString(), "True", Session["UserId"].ToString(), DateTime.Now.ToString(), ref trns);
                ...
                foreach (GridViewRow gvr in GvProduct.Rows)
                {
                    ...
                    if (ChkPost.Checked)
                    {
                        ...
                        {
                            DataTable dt = (DataTable)Session["dtFinal"];
                            dt = new DataView(dt, "ProductId='" + lblgvProductId.Text + "' and InOut='" + hdngvInOut.Value + "'", "", DataViewRowState.CurrentRows).ToTable();
            ...
            else
            {
                ...
                b = objAdjustHeader.InsertAdjustHeader(Session["CompId"].ToString(), Session["BrandId"].ToString(), strLocationId, strLocationId, txtVoucherNo.Text, ObjSysParam.getDateForInput(txtVoucherDate.Text).ToString(), txtRemark.Text, txtNetAmount.Text, strPost, hdnSinvId.Value, txtSinvRefOrderId.Text, txtSinvRefInvoiceNo.Text, "", "", "True", DateTime.Now.ToString(), "True", Session["UserId"].ToString(), DateTime.Now.ToString(), Session["UserId"].ToString(), DateTime.Now.ToString(), ref trns);
                    ...
                    foreach (GridViewRow gvr in GvProduct.Rows)
                    {
                        ...
                        if (ChkPost.Checked)
                        {
                            ...
                            if (Session["dtFinal"] != null)
                            {
                                ...
                                DataTable dtReplaced = new DataTable();
                                if (hdnSinvId.Value != "0" && !string.IsNullOrEmpty(txtSinvNo.Text) && dtSinvSerial.Rows.Count > 0)
                                {
                                    dtReplaced = new DataView((DataTable)Session["dtFinal"], "ProductId='" + lblgvProductId.Text + "' and InOut='I'", "", DataViewRowState.CurrentRows).ToTable();
                                }
                                string updatedSbTransId = "0";
                                ...
                                foreach (DataRow dr in dt.Rows)
                                {
                                    ...
                                    if (hdngvInOut.Value.Trim() == "O")
                                    {
                                        ...
                                        //update same serial in reference of sales invoice, if it is replacement 
                                        if (hdnSinvId.Value != "0" && !string.IsNullOrEmpty(txtSinvNo.Text) && dtSinvSerial.Rows.Count > 0)
                                        {
                                            if (dtReplaced.Rows.Count > 0)
                                            {
                                                foreach (DataRow drReplaced in dtReplaced.Rows)
                                                {
                                                    using (DataTable dtInvoicedSerial = new DataView(dtSinvSerial, "productId=" + ((Label)gvr.FindControl("lblgvProductId")).Text + " and serialNo='" + drReplaced["SerialNo"].ToString() + "' and trans_id not in(" + updatedSbTransId + ")", "", DataViewRowState.CurrentRows).ToTable())
                                                    {
                                                        if (dtInvoicedSerial.Rows.Count > 0)
                                                        {
                                                            string strSbTransId = "";
                                                            strSbTransId = dtInvoicedSerial.Rows[0]["Trans_id"].ToString();
                                                            string sqlNew = "update Inv_StockBatchMaster set serialNo='" + dr["serialNo"].ToString() + "' where trans_id=" + strSbTransId;
                                                            objDa.execute_Command(sqlNew, ref trns);
                                                            updatedSbTransId += "," + strSbTransId;
                                                            continue;
                                                        }
                                                    }
                                                }
    ...
    private void FillGrid()
    {
        ...
        {
            PostStatus = " Post='True' and FromLocationID=" + Session["LocId"].ToString() + "";
        }
        else if (ddlPosted.SelectedItem.Value == "UnPosted")
        {
            PostStatus = " Post='False' and FromLocationID=" + Session["LocId"].ToString() + "";
        }
        else
        {
            PostStatus = " FromLocationID=" + Session["LocId"].ToString() + "";
        }
    ...
    public void Reset()
    {
        ...
        Session["dtFinal"] = null;
        Session["dtSerial"] = null;
        txtSinvNo.Text = txtCustomerName.Text = txtSinvQty.Text = txtSinvRefOrderId.Text = txtSinvRefInvoiceNo.Text = string.Empty;
        hdnSinvId.Value = "0";
        txtSinvNo.Enabled = true;
        ...
        {
            //DataTable dtAllPageCode = cmn.GetAllPagePermission(Session["UserId"].ToString(), ViewState["ModuelId"].ToString(), "131");
            //try
            //{
    ...
    [System.Web.Services.WebMethodAttribute(), System.Web.Script.Services.ScriptMethodAttribute()]
    public static string[] GetCompletionListSinvNo(string prefixText, int count, string contextKey)
    {
        DataAccessClass daClass = new DataAccessClass(HttpContext.Current.Session["DBConnection"].ToString());
        PassDataToSql[] paramList = new PassDataToSql[1];
        paramList[0] = new PassDataToSql("@sinvNo", prefixText, PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
        string sql = "select top 20 invoice_no from Inv_SalesInvoiceHeader where Location_Id='" + HttpContext.Current.Session["LocId"].ToString() + "' and Post='true' and IsActive='true' and Invoice_No like  '%@sinvNo%'";
        string[] txt = null;
        using (DataTable _dt = daClass.GetDtFromQryWithParam(sql, paramList))
        {
            if (_dt.Rows.Count > 0)
            {
                txt = new string[_dt.Rows.Count];
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    txt[i] = _dt.Rows[i]["invoice_no"].ToString();
                }
            }
        }
        return txt;
    ...
    protected void BtnSerialSave_Click(object sender, EventArgs e)
    {
                    ...
                    if (txt[i].ToString().Trim() != "")
                    {
                        if ((new DataView(dt, "SerialNo='" + txt[i].ToString().Trim() + "'", "", DataViewRowState.CurrentRows).ToTable().Rows.Count) == 0)
                        {
                            ...
                            if (result[0] == "VALID")
                            {
                                ...
                                dr[10] = "1";
                                dr["InOut"] = Session["InOut"].ToString();
                                counter++;
        ...
        else
        {
            ...
            Dtfinal = new DataView(Dtfinal, "ProductId<>'" + Session["PID"].ToString() + "' or InOut<>'" + Session["InOut"].ToString() + "'", "", DataViewRowState.CurrentRows).ToTable();
    ...
    protected void lnkAddSerial_Command(object sender, CommandEventArgs e)
    {
        ...
        else
        {
            ...
            dt = new DataView(dt, "ProductId='" + Session["PID"].ToString() + "' and InOut='" + Session["InOut"].ToString() + "'", "", DataViewRowState.CurrentRows).ToTable();
    ...
    protected void txtGVQuantity_OnTextChanged(object sender, EventArgs e)
    {
        ...
        if (((Label)GvRow.FindControl("lblgvUnitCost")).Text == "")
        {
            ((Label)GvRow.FindControl("lblgvUnitCost")).Text = "0";
        }

        ((Label)GvRow.FindControl("lblgvTotal")).Text = GetAmountDecimal((Convert.ToDouble(((TextBox)sender).Text) * Convert.ToDouble(((Label)GvRow.FindControl("lblgvUnitCost")).Text)).ToString());
    ...
    {
        hdnPrintId.Value = e.CommandArgument.ToString();
        hdnRPTClick.Value = "1";
        GetReportStatement();
    }
    public void GetReportStatement()
    {
        ...
        InventoryDataSetTableAdapters.sp_Inv_AdjustHeader_ReportTableAdapter adp = new InventoryDataSetTableAdapters.sp_Inv_AdjustHeader_ReportTableAdapter();
      
        adp.Connection.ConnectionString = Session["DBConnection"].ToString();
        adp.Fill(rptdata.sp_Inv_AdjustHeader_Report, int.Parse(hdnPrintId.Value.ToString()));
        dtFilter = rptdata.sp_Inv_AdjustHeader_Report;
        string CompanyName = "";
        ...
        stockReport.setcompanyname(CompanyName);
        stockReport.setCompanyAddress(CompanyAddress);
        stockReport.setCompanyLogo(Imageurl);

        stockReport.DataSource = dtFilter;
        stockReport.DataMember = "sp_Inv_AdjustHeader_Report";
        stockReport.CreateDocument();
        ReportViewer1.OpenReport(stockReport);
        dtFilter.Dispose();
    }
    ...
    protected void txtSinvNo_TextChanged(object sender, EventArgs e)
    {
        ...
        txtSinvQty.Text = txtCustomerName.Text = string.Empty;
        ddlSinvProduct.Items.Clear();
        if (string.IsNullOrEmpty(txtSinvNo.Text))
        {
        ...
        PassDataToSql[] paramList = new PassDataToSql[1];
        paramList[0] = new PassDataToSql("@sinvNo", txtSinvNo.Text, PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
        string sql = "select sid.Product_Id,pm.ProductCode,cm.Name,sih.Trans_Id from Inv_SalesInvoiceDetail sid inner join Inv_SalesInvoiceHeader sih on sih.Trans_Id=sid.Invoice_No inner join Inv_ProductMaster pm on sid.Product_Id=pm.ProductId inner join Ems_ContactMaster cm on sih.Supplier_Id=cm.Trans_Id where sid.IsActive='true' and sih.Invoice_No=@sinvNo";
        using (DataTable _dt = objDa.GetDtFromQryWithParam(sql, paramList))
        {
            if (_dt.Rows.Count > 0)
            {
                hdnSinvId.Value = _dt.Rows[0]["Trans_id"].ToString();
                txtCustomerName.Text = _dt.Rows[0]["Name"].ToString();
                objPageCmn.FillData(ddlSinvProduct, _dt, "ProductCode", "Product_Id");
            }
            else
            {
    ...
    protected void ddlSinvProduct_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    protected void btnReplacement_Click(object sender, EventArgs e)
    {
        ...
        if (ddlSinvProduct.SelectedValue == "--Select--")
        {
            DisplayMessage("Please select valid product");
            return;
        }

        double replaceQty = 0;
        double.TryParse(txtSinvQty.Text, out replaceQty);
        if (replaceQty == 0)
        {
        ...
        try
        {
            ...
            PassDataToSql[] paramList = new PassDataToSql[1];
            paramList[0] = new PassDataToSql("@sinvNo", txtSinvNo.Text, PassDataToSql.ParaTypeList.Nvarchar, PassDataToSql.ParaDirectonList.Input);
            string sql = "select sid.Product_Id,pm.ProductCode,cm.Name,sid.UnitPrice,sid.Unit_Id,sid.Quantity,pm.EProductName from Inv_SalesInvoiceDetail sid inner join Inv_SalesInvoiceHeader sih on sih.Trans_Id=sid.Invoice_No inner join Inv_ProductMaster pm on sid.Product_Id=pm.ProductId inner join Ems_ContactMaster cm on sih.Supplier_Id=cm.Trans_Id where sid.IsActive='true' and sih.Invoice_No=@sinvNo";
            using (DataTable _dt = objDa.GetDtFromQryWithParam(sql, paramList))
            {
                DataTable dtTemp = CreateProductDataTable();
                if (_dt.Rows.Count > 0)
                {
                    //add row for stock in
                    dtTemp.Rows.Add(0);
                    dtTemp.Rows[0]["SerialNo"] = "1";
                    dtTemp.Rows[0]["ProductId"] = _dt.Rows[0]["Product_id"].ToString();
                    dtTemp.Rows[0]["UnitID"] = _dt.Rows[0]["Unit_Id"].ToString();
                    double invoicedQty = 0;
                    double.TryParse(_dt.Rows[0]["quantity"].ToString(), out invoicedQty);
                    if (replaceQty > invoicedQty)
                    {
                        DisplayMessage("You can relace more then invoiced qty");
                        return;
                    }

                    dtTemp.Rows[0]["Quantity"] = replaceQty.ToString();
                    dtTemp.Rows[0]["UnitCost"] = _dt.Rows[0]["UnitPrice"].ToString();
                    dtTemp.Rows[0]["InOut"] = "I";

                    //add row for stock out
                    dtTemp.Rows.Add(0);
                    dtTemp.Rows[1]["SerialNo"] = "2";
                    dtTemp.Rows[1]["ProductId"] = _dt.Rows[0]["Product_id"].ToString();
                    dtTemp.Rows[1]["UnitID"] = _dt.Rows[0]["Unit_Id"].ToString();
                    dtTemp.Rows[1]["Quantity"] = replaceQty.ToString();
                    dtTemp.Rows[1]["UnitCost"] = _dt.Rows[0]["UnitPrice"].ToString();
                    dtTemp.Rows[1]["InOut"] = "O";

                    //fill grid
                    objPageCmn.FillData((object)GvProduct, dtTemp, "", "");
                    ...
                    //disable edit and delete in case of replacement
                    GvProduct.Columns[0].Visible = GvProduct.Columns[1].Visible = false;
