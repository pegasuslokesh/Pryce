        ...
        $.ajax({
            ...
            success: function (ab) {
                ...
                $(result).each(function (key, item) {
                    ...
                    htmlRow = htmlRow + "<td style='text-align:right'><span class='badge bg-yellow'>" + item.stock_qty + "</span></td>";
                    htmlRow = htmlRow + "</tr>";
                    $('#tblLocation > tbody').append(htmlRow);
                    totalValue += parseFloat(item.stock_qty);
                });
