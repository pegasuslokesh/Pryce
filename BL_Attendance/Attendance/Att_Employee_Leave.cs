...
public class Att_Employee_Leave
{
    ...
    public DataTable GetLeaveType()
    {
        ...
        string sql = "Select Leave_Name,Leave_Id from Att_LeaveMaster where IsActive=1 And Att_LeaveMaster.Field1='True'";
        dtLeave = daClass.return_DataTable(sql);
        return dtLeave;
    }
    ...
    public int UpdateEmployeeLeaveTransactionByTransNo(string TransId, string CompanyId, string Emp_Id, string leavetypeid, string Year, string Month, string Previous_Days, string Assign_Days, string Total_Days, string Used_Days, string Remaining_Days, string Pending_Days, string RemainPaidDays, string ModifiedBy, string ModifiedDate, string strEncashDays, ref SqlTransaction trns)
    {
        ...
        paramList[10] = new PassDataToSql("@Encash_Days", strEncashDays, PassDataToSql.ParaTypeList.Int, PassDataToSql.ParaDirectonList.Input);
    ...
    public DataTable GetEmployeeLeaveTransactionDataByEmpIdForAnnualLeave(string empid)
    {
        ...
        dtInfo = daClass.Reuturn_Datatable_Search("Att_Employee_Leave_Trans_SelectByEmpIdForAnnualLeave", paramList);
        return dtInfo;
    }


    public DataTable GetEmployeeLeaveTransactionAll( string Sql)
    {
        DataTable dtInfo = new DataTable();        
        dtInfo = daClass.return_DataTable(Sql);
